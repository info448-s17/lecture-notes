...
## Hello World `r #30min`
As a final introductory steps, this chapter will walk you through creating and running a basic App so that you can see what you will actually be working with. You will need to have Android Studio installed for this to work.
...
3. Select an _Empty Activity_
	...
	- **Activities** are "Screens" in your application (things the user can do). Activities are discussed in more detail in the next chapter.
...
Note that Android Studio by default shows the **"Android"** view, which organizes files thematically. If you go to the **"Project"** view you can see what the actual file system looks like:
...
- `res` has resource files. These are **XML** files that specify details of the app--such as layout.
	...
	See also: http://developer.android.com/guide/topics/resources/available-resources.html, or Chapter 3.

We can also consider what the application code does. While we'll revisit this in more detail in the next chapter, it's useful to start seeing how the framework is structured:
...
In this class, we override the `onCreate()` method that is called by the framework when the Activity starts (see next chapter).
...
Finally, as a fun demonstration, try to set an icon for the App (`File > New > Image Asset`)
