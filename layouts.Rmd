...
All views have three basic aspects:

- They have **properties** which define the state of the View
- They have **methods** which the Java code can call to manipulate them ("outside to inside" communication)
- They can produce **events** which they can use to notify the Java code ("inside to outside" communication)

This lecture will primarily focus on the **properties**. Properties are usually defined within the resource XML as element _attributes_. Some examples of these property attributes are described below.
